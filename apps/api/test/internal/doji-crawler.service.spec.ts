import { DojiCrawlerService } from '../../src/crawler/doji-crawler.service';
...
describe('DojiCrawlerService.onModuleInit', () => {
  it('registers crawler when scheduler is provided', () => {
    const scheduler = { registerCrawler: jest.fn() } as any;
    const service = new DojiCrawlerService(mockPrisma, mockDetector, scheduler);

    service.onModuleInit();

    expect(scheduler.registerCrawler).toHaveBeenCalledWith(
      'DOJI',
      expect.any(Function),
    );
  });

  it('does not throw when scheduler is missing', () => {
    const service = new DojiCrawlerService(mockPrisma, mockDetector);
