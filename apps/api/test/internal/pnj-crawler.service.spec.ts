import { PnjCrawlerService } from '../../src/crawler/pnj-crawler.service';
...
describe('PnjCrawlerService.onModuleInit', () => {
  it('registers crawler when scheduler is provided', () => {
    const scheduler = { registerCrawler: jest.fn() } as any;
    const service = new PnjCrawlerService(mockPrisma, mockDetector, scheduler);
  ...
  it('does not throw when scheduler is missing', () => {
    const service = new PnjCrawlerService(mockPrisma, mockDetector);
