...
import { HeatIndexService } from '../../src/heat-index/heat-index.service';
...
describe('HeatIndexService.compute()', () => {
  ...
  it('no records → zeroed scores and Cold category', async () => {
    mockPrisma.priceRecord.findMany
  ...
  describe('getCurrent and history helpers', () => {
    it('getCurrent computes and saves when no latest exists', async () => {
      const now = new Date('2026-05-20T10:00:00Z');
      jest.useFakeTimers().setSystemTime(now);

      mockPrisma.heatIndexRecord.findFirst.mockResolvedValue(null);
      ...
      mockPrisma.heatIndexRecord.create.mockResolvedValue({
        indexValue: 25,
        category: 'Cold',
        priceVelocity: 10,
        spreadSize: 200000n,
      ...
      expect(mockPrisma.heatIndexRecord.create).toHaveBeenCalled();
      expect(result.value).toBe(25);

      jest.useRealTimers();
    });
    it('getCurrent returns latest when record is fresh', async () => {
      const now = new Date('2026-05-20T10:00:00Z');
      jest.useFakeTimers().setSystemTime(now);
      ...
      const latest = {
        ...
        calculatedAt: new Date(now.getTime() - 5 * 60_000),
      };
      mockPrisma.heatIndexRecord.findFirst.mockResolvedValue(latest);
    ...
    it('getCurrent recomputes when record is stale', async () => {
      const now = new Date('2026-05-20T10:00:00Z');
      jest.useFakeTimers().setSystemTime(now);

      mockPrisma.heatIndexRecord.findFirst.mockResolvedValue({
        indexValue: 10,
        category: 'Cold',
        priceVelocity: 10,
        spreadSize: 200000n,
        thresholdCrossings: 1,
        calculatedAt: new Date(now.getTime() - 20 * 60_000),
      });
    ...
    it('getHistory returns mapped DTOs', async () => {
      const records = [
        {
          indexValue: 40,
          category: 'Warm',
      ...
      mockPrisma.heatIndexRecord.findMany.mockResolvedValue(records);

      const result = await service.getHistory(3);

      expect(result[0].value).toBe(40);
      expect(result[0].calculatedAt).toBe('2026-05-18T00:00:00.000Z');
    });

    it('computeAndStore saves computed record', async () => {
      jest.spyOn(service, 'compute').mockResolvedValue({
        indexValue: 12,
        category: 'Cold',
        priceVelocity: 10,
      ...
      await service.computeAndStore();

      expect(mockPrisma.heatIndexRecord.create).toHaveBeenCalledWith({
        data: expect.objectContaining({ indexValue: 12 }),
      });
    });
  });
});
