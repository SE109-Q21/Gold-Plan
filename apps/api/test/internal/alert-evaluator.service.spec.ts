...
describe('AlertEvaluatorService', () => {
  ...
  it('repeatMode=true with no lastTriggeredAt → fires and sets lastTriggeredAt', async () => {
    const alert = makeAlert({
      condition: 'lte',
      thresholdPrice: 80_000_000n,
      repeatMode: true,
  ...
  it('repeatMode=true with cooldown exactly 30 minutes → fires', async () => {
    const now = new Date('2026-05-20T10:00:00Z');
    jest.useFakeTimers().setSystemTime(now);
    const boundary = new Date(now.getTime() - 30 * 60_000);
    const alert = makeAlert({
      condition: 'lte',
      thresholdPrice: 80_000_000n,
      repeatMode: true,
      lastTriggeredAt: boundary,
    });
    const record = makePriceRecord(79_000_000n);
  ...
  it('no active alerts → no work', async () => {
  ...
  it('gte condition NOT met → no action', async () => {
    const alert = makeAlert({ condition: 'gte', thresholdPrice: 80_000_000n, repeatMode: false });
    const record = makePriceRecord(79_000_000n);
