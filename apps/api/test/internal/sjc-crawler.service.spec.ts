import { SjcCrawlerService } from '../../src/crawler/sjc-crawler.service';
...
describe('SjcCrawlerService.onModuleInit', () => {
  it('registers crawler when scheduler is provided', () => {
    const scheduler = { registerCrawler: jest.fn() } as any;
    const service = new SjcCrawlerService(mockPrisma, mockDetector, scheduler);
  ...
  it('does not throw when scheduler is missing', () => {
    const service = new SjcCrawlerService(mockPrisma, mockDetector);
